    ...
    <div class="app-container" :class="{ [$style.imageLoaded]: isImageLoaded }">
        ...
        <div :class="$style.controls">
            <div ref="controlsContainer" :class="$style.controlsContainer">
                ...
                >
                    ...
                    <div v-show="activeControlsTab === 1">
                        <div class="controls-tab-container">
                                ...
                                >
                                    <div class="label-align">
                                        ...
                                        >
                                            <option
                                                v-for="(
                                                    smoothingValue, index
                                                ) in imageSmoothingValues"
                                                :value="index"
                                                :key="index"
                                            >
                                                {{
                            ...
                            <outline-filters-controls
                                ...
                                :display-image="zoomImage"
                                :is-image-outline-filter-enabled="
                                    isImageOutlineFilterEnabled
                                "
                                :is-color-picker-live-preview-enabled="
                                    isColorPickerLivePreviewEnabled
                                "
                                :request-outline-display="
                                    onOutlineFilterDisplayRequested
                                "
                                :request-resources-for-outline="
                                    onResourcesForOutlineFilterRequested
                                "
                            />
                        </div>
                    </div>
                    ...
                    <div v-show="activeControlsTab === 2">
                        <settings-tab
                            ...
                            "
                            v-model:is-live-preview-enabled="
                                isLivePreviewEnabled
                            "
                            v-model:is-color-picker-live-preview-enabled-setting="
                                isColorPickerLivePreviewEnabledSetting
                            "
                            v-model:automatically-resize-large-images="
                                automaticallyResizeLargeImages
                            "
                ...
                >
                    ...
                    />
                    <div
                        v-show="activeDitherComponentId === bwDitherComponentId"
                    >
                        <bw-dither-section
                            ref="bwDitherSection"
                            @request-worker="onWorkerRequested"
                            :request-canvases="
                                requestPermissionCallbackBuilder(
                                    bwDitherComponentId,
                                    onCanvasesRequested
                                )
                            "
                            :request-display-transformed-image="
                                requestPermissionCallbackBuilder(
                                    bwDitherComponentId,
                                    onRequestDisplayTransformedImage
                                )
                            "
                            :is-webgl-enabled="isWebglEnabled"
                            :is-live-preview-enabled="
                                isLivePreviewEnabled || isBatchConverting
                            "
                            :is-color-picker-live-preview-enabled="
                                isColorPickerLivePreviewEnabled
                            "
                    ...
                    <div
                        v-show="
                            activeDitherComponentId === colorDitherComponentId
                        "
                    >
            ...
            <zoom-bar
                ...
                :show-original-image="showOriginalImage"
                :zoom-changed="zoomImage"
                :request-dimensions-for-zoom-fit="
                    onDimensionsRequestedForZoomFit
                "
            />
        </div>
        ...
        <div ref="unsplashAttributionContainer">
            <unsplash-attribution
                v-if="loadedImage && loadedImage.unsplash"
                :unsplash-info="loadedImage.unsplash"
            />
        </div>
...
export default {
    ...
    created() {
        ...
        if (transformCanvasWebGl.gl) {
            ...
            this.isWebglHighpFloatSupported =
                transformCanvasWebGl.supportsHighFloatPrecision;
    ...
    mounted() {
        ...
        sourceCanvasOutput = Canvas.create(
            refs.imageCanvasSupercontainer.sourceCanvasOutput
        );
        transformCanvasOutput = Canvas.create(
        ...
        if (this.isWebglSupported) {
            this.$refs.alertsContainer.webglMaxTextureSize =
                transformCanvasWebGl.maxTextureSize;
        }

        //load global settings
        const globalSettings = UserSettings.getGlobalSettings(
            this.areControlsPinned()
        );
        this.currentEditorThemeIndex = EditorThemes.indexForKey(
            this.editorThemes,
            globalSettings.editorThemeKey
        );
        this.showOriginalImage = globalSettings.showOriginalImage;
        this.isLivePreviewEnabled = globalSettings.isLivePreviewEnabled;
        this.isColorPickerLivePreviewEnabledSetting =
            globalSettings.isColorPickerLivePreviewEnabled;
        this.automaticallyResizeLargeImages =
            globalSettings.automaticallyResizeLargeImages;
        this.isWebglEnabled =
            this.isWebglSupported && globalSettings.isWebglEnabled;
    ...
    data() {
        return {
            ...
            selectedImageSaturationIndex:
                ImageFiltersModel.canvasFilterValuesDefaultIndex,
            selectedImageContrastIndex:
                ImageFiltersModel.canvasFilterValuesDefaultIndex,
            selectedImageBrightnessIndex:
                ImageFiltersModel.canvasFilterValuesDefaultIndex,
    ...
    computed: {
        ...
        imageHeader() {
            ...
            let width = Math.ceil(
                this.loadedImage.width * (this.pixelateImageZoom / 100)
            );
            let height = Math.ceil(
                this.loadedImage.height * (this.pixelateImageZoom / 100)
            );
            return {
                width: width,
                height: height,
        ...
        pixelateImageZooms() {
            const dimensions = this.isImageLoaded
        ...
        imageFiltersRaw() {
            const filters = {};
            const contrast =
                this.canvasFilterValues[this.selectedImageContrastIndex];
            const saturation =
                this.canvasFilterValues[this.selectedImageSaturationIndex];
            const brightness =
                this.canvasFilterValues[this.selectedImageBrightnessIndex];
        ...
        serializedGlobalSettings() {
            const editorThemeKey =
                this.currentEditorThemeIndex === null
                    ? DEFAULT_EDITOR_THEME_KEY
                    : this.editorThemes[this.currentEditorThemeIndex].key;
    ...
    watch: {
        currentEditorThemeIndex(newThemeIndex, oldThemeIndex) {
            ...
            if (oldThemeIndex !== null) {
                const oldThemeClass =
                    this.editorThemes[oldThemeIndex].className;
    ...
    methods: {
        ...
        loadDitherTab(componentId) {
            ...
            if (this.isImageLoaded) {
                const hasSeenImage = tabsThatHaveSeenImageSet.has(
                    this.activeDitherComponentId
                );
                tabsThatHaveSeenImageSet.add(this.activeDitherComponentId);
                this.activeDitherSection.imageLoaded(
                    this.imageHeader,
                    !hasSeenImage
                );
            }
        ...
        loadImage(image, file, dimensions) {
            ...
            const largestImageDimension = Math.max(
                loadedImage.width,
                loadedImage.height
            );
            if (
        ...
        imageFiltersBeforeDitherChanged(notifyDitherSection = true) {
            ...
            if (this.isWebglEnabled) {
                ...
                if (hasImageBeenTransformed) {
                    sourceCanvas.context.drawImage(
            ...
            const ditherWorkerMessage = WorkerUtil.createLoadImageMessage(
                imageId,
        ...
        imagePixelationChanged() {
            ...
            const filters = this.areCanvasFiltersSupported
                ? this.imageFilters
                : '';
            Canvas.copy(
                originalImageCanvas,
            ...
            if (
                this.areCanvasFiltersSupported &&
                transformCanvas.context.filter
            ) {
        ...
        bilateralFilterValueBeforeChanged() {
            const filterExponent =
                this.bilateralFilterValues[
                    this.selectedBilateralFilterValueBefore
                ];
            if (filterExponent < 0) {
                return false;
            }
            const imageHeader = this.imageHeader;
            sourceWebglTexture = WebGlBilateralFilter.filterImage(
        ...
        imageSmoothingBeforeChanged() {
            const smoothingRadius =
                this.imageSmoothingValues[
                    this.selectedImageSmoothingRadiusBefore
                ];
        ...
        imageSmoothingAfterChanged() {
            const smoothingRadius =
                this.imageSmoothingValues[
                    this.selectedImageSmoothingRadiusAfter
                ];
            if (!this.isSmoothingEnabled || smoothingRadius <= 0) {
                return false;
            }
        ...
        bilateralFilterValueAfterChanged() {
            const filterExponent =
                this.bilateralFilterValues[
                    this.selectedBilateralFilterValueAfter
                ];
            if (filterExponent < 0) {
                return false;
            }
            const imageHeader = this.imageHeader;
            ditherOutputWebglTexture = WebGlBilateralFilter.filterImage(
                transformCanvasWebGl,
                ditherOutputWebglTexture,
                imageHeader.width,
                imageHeader.height,
        ...
        imageFiltersAfterDitherChanged() {
            ...
            ditherOutputWebglTexture = WebGl.createAndLoadTextureFromCanvas(
                transformCanvasWebGl.gl,
                ditherOutputCanvas.canvas
            );
            let hasImageBeenTransformed = false;
            hasImageBeenTransformed =
                this.bilateralFilterValueAfterChanged() ||
                hasImageBeenTransformed;
            hasImageBeenTransformed =
                this.imageSmoothingAfterChanged() || hasImageBeenTransformed;
            if (hasImageBeenTransformed) {
        ...
        onResourcesForOutlineFilterRequested(callback) {
            ...
            callback(
                ...
                ditherOutputWebglTexture,
                transformCanvasWebGl,
                this.$refs.colorDitherSection.selectedColorsVec
            );
        },
        areControlsPinned() {
            return (
        ...
        zoomImage() {
            const scaleAmount =
                this.$refs.zoomBar.zoom / this.pixelateImageZoom;
        ...
        onDimensionsRequestedForZoomFit(callback) {
            ...
            const controlsContainerWidth = areControlsPinned
                ? this.$refs.controlsContainer.offsetWidth
                : 0;
            const canvasMargin = this.showOriginalImage
                ? parseInt(
                      getComputedStyle(
                          this.$refs.imageCanvasSupercontainer
                              .sourceCanvasOutput
                      )
        ...
        workerMessageReceivedDispatcher(e) {
            ...
            switch (messageTypeId) {
                ...
                case WorkerHeaders.HISTOGRAM:
                    this.$refs.bwDitherSection.ditherWorkerMessageReceivedDispatcher(
                        messageTypeId,
                        messageData
                    );
                    break;
