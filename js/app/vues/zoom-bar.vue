...
.zoomLabel {
    ...
    max-width: calc(
        100vw - #{variables.$chrome_fullscreen_horizontal_scrollbar_height}
    );
    input[type='range'] {
        flex-grow: 1;
...
function maxScalePercentageForImage(
    imageWidth,
    imageHeight,
    maximumAllowedSize
) {
    const largestDimension = Math.max(imageWidth, imageHeight);
    const largestPercentage = Math.ceil(
        (maximumAllowedSize * 100) / largestDimension
    );
...
function minScalePercentageForImage(
    imageWidth,
    imageHeight,
    minimumAllowedSize
) {
    const smallestDimension = Math.min(imageWidth, imageHeight);
    const smallestPercentage = Math.ceil(
        (minimumAllowedSize * 100) / smallestDimension
    );
...
export default {
    ...
    computed: {
        zoomMin() {
            ...
            const smallestDimension = Math.floor(
                Math.min(window.innerHeight, window.innerWidth) /
                    2 /
                    devicePixelRatio
            );
            return minScalePercentageForImage(
                this.image.width,
                this.image.height,
                Math.min(100, smallestDimension)
            );
        },
        zoomMax() {
            if (!this.isImageLoaded) {
            ...
            const greatestDimension =
                Math.max(window.innerHeight, window.innerWidth) *
                2 *
                devicePixelRatio;
            return maxScalePercentageForImage(
                this.image.width,
                this.image.height,
                greatestDimension
            );
    ...
    watch: {
        zoomDisplay(newValue) {
            ...
            if (
                newValue !== this.zoom &&
                newValue >= this.zoomMin &&
                newValue <= this.zoomMax
            ) {
                this.zoom = newValue;
            }
        },
    ...
    methods: {
        ...
        zoomFit() {
            ...
            this.requestDimensionsForZoomFit(
                (areControlsPinned, controlsContainerWidth, canvasMargin) => {
                    ...
                    if (
                        getComputedStyle(zoomBarContainer).getPropertyValue(
                            'position'
                        ) === 'fixed'
                    ) {
                    ...
                    const widthFitPercentage = Math.floor(
                        (windowWidth / image.width) * widthMultiplier
                    );
                    const heightFitPercentage = Math.floor(
                        (windowHeight / image.height) * 100
                    );
                    this.zoom = Math.min(
                        widthFitPercentage,
                        heightFitPercentage
