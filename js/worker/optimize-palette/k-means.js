...
import OptimizePalettePerceptual from './perceptual.js';
...
function convergeMeans(paletteBuffer, meansBuffer, numColors) {
    ...
    for (
        let i = 0, paletteBufferIndexOffset = 0, meansBufferIndexOffset = 0;
        i < numColors;
        i++, paletteBufferIndexOffset += 3, meansBufferIndexOffset += 4
    ) {
        const numPixels = meansBuffer[meansBufferIndexOffset + 3];
        for (let j = 0; j < 3; j++) {
            const mean = Math.round(
                meansBuffer[meansBufferIndexOffset + j] / numPixels
            );
...
function kMeans(
    ...
    const paletteBuffer = OptimizePalettePerceptual.medianCut(
        pixels,
        numColors,
        { hueMix: 1.6 },
        imageWidth,
    ...
    for (
        let currentIteration = 0, hasConverged = false;
        !hasConverged && currentIteration < maximumIterations;
        currentIteration++
    ) {
        colorHistogram.forEach((count, color32) => {
            ...
            for (let j = 0; j < 3; j++) {
                meansBuffer[meansBufferIndexOffset + j] +=
                    pixelBuffer[j] * count;
            }
