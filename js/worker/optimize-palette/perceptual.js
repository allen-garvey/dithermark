...
function createHslPopularityMap(pixels, hueClamp = false) {
    ...
    Image.forEachOpaquePixel(pixels, pixel => {
        ...
        const lightnessDiff =
            lightness >= 128 ? lightness - 128 : 127 - lightness;
        const hueCountValue =
            ((saturation * saturation * saturation) / 1000000) *
            ((maxLightnessDiffCubed -
                lightnessDiff * lightnessDiff * lightnessDiff) /
                maxLightnessDiffCubed);
...
function sortHuesByClosestLightness(hues, pixels) {
    ...
    Image.forEachOpaquePixel(pixels, pixel => {
        ...
        hueAverageLigtnessMap[normalizedIndex] =
            hueAverageLigtnessMap[normalizedIndex] + lightness * fraction;
        hueAverageLigtnessMap[normalizedIndex + 1] =
            hueAverageLigtnessMap[normalizedIndex + 1] + fraction;
    });
...
function medianPopularityBase(
    popularityMapObject,
    numColors,
    numDistinctValues,
    bucketCapacityFunc = null
) {
    let popularityMap = popularityMapObject.map.slice();

    let bucketsAverage =
        numDistinctValues <= 255
            ? new Uint8Array(numColors)
            : new Uint16Array(numColors);
    ...
    for (
        let bucketIndex = 0;
        bucketIndex < bucketsAverage.length;
        bucketIndex++
    ) {
        let bucketCount = 0;
        let bucketTotal = 0;
        let isLastBucket = bucketIndex === bucketsAverage.length - 1;
        bucketMaxCapacity = bucketCapacityFunc(
            numPixelsUsed,
            bucketsAverage.length,
            bucketIndex,
            bucketMaxCapacity
        );
        ...
        bucketsMedian[bucketIndex] = Math.round(
            (currentMedianValue - previousMedianValue) / 2 + previousMedianValue
        );
...
function medianPopularityBase2(
    popularityMapObject,
    numColors,
    numDistinctValues,
    offset = 0
...
function medianPopularityHues(popularityMap, numColors) {
    ...
    for (let i = 0; i < numValues; i++) {
        ...
        let results = medianPopularityBase2(
            popularityMap,
            numColors,
            numValues,
            i
...
function calculatedPopularityHues(popularityMapObject, numColors) {
    ...
    const longestZeroSequence = findLongestPopularityMapZeroSequence(
        popularityMapObject.map,
        Math.max(threshold, 0)
    );
    const startIndex =
        longestZeroSequence.startIndex + longestZeroSequence.length;
    popularityMapObject.map = doubledPopularityMap.subarray(
        startIndex,
        startIndex + numDistinctValues
    );
    const ret = medianPopularityBase2(
        popularityMapObject,
        numColors,
        numDistinctValues,
        startIndex
...
function uniformDistribution(
    ...
    let buckets =
        valueMax <= 255
            ? new Uint8Array(numDivisions)
            : new Uint16Array(numDivisions);
    const divisor = isCircular ? numDivisions : numDivisions - 1;
    const bucketFraction = (valueMax - valueMin) / divisor;

    buckets = buckets.map((value, i) => {
        return Math.round(i * bucketFraction) + valueMin;
    });
    return buckets;
}
...
function findLongestPopularityMapZeroSequence(
    popularityMap,
    thresholdValue = 0
) {
...
function hueUniformPopularity(popularityMapObject, numColors) {
    ...
    const baseUniformDistribution = uniformDistribution(
        startIndex,
        endIndex,
        numColors,
        isCircular
...
function lightnessUniformPopularity(popularityMapObject, numColors) {
    ...
    const bottomOffset = Math.ceil(numDistinctValues * 0.18);
    const topOffset = Math.ceil(numDistinctValues * 0.18);
...
function zipHsl(hues, saturations, lightnesses, numColors) {
    ...
    //lightness
    for (
        let retIndex = 2, lightnessIndex = 0;
        retIndex < ret.length;
        retIndex += 3, lightnessIndex++
    ) {
    ...
    for (
        let retIndex = 1, saturationIndex = 0;
        retIndex < half;
        retIndex += 3, saturationIndex += 2
    ) {
...
function averageArrays(array1, array2, weight = 1) {
    ...
    return array1.map((value, index) => {
        return Math.floor((value * weight + array2[index] * counterWeight) / 2);
    });
}
...
function perceptualMedianCut(
    ...
    let logarithmicBucketCapacityFunc = (
        numPixels,
        numBuckets,
        currentBucketNum,
        previousBucketCapacity
        ...
        previousBucketCapacity =
            previousBucketCapacity > 0 ? previousBucketCapacity : numPixels;
        return Math.ceil(previousBucketCapacity / Math.LN10);
    };
    //lightness
    let lightnessesPopularityMapObject = createPopularityMap(
        pixels,
        256,
        PixelMath.lightness
    );
    let medianLightnesses = medianPopularityBase(
        lightnessesPopularityMapObject,
        numColors,
        256
    );
    let uniformLightnesses = lightnessUniformPopularity(
        lightnessesPopularityMapObject,
        numColors
    );
    let lightnesses = averageArrays(
        medianLightnesses.average,
        uniformLightnesses,
        0.8
    );

    //saturation
    let saturationsPopularityMapObject = createPopularityMap(
        pixels,
        101,
        PixelMath.saturation
    );
    let medianSaturations = medianPopularityBase(
        saturationsPopularityMapObject,
        numColors,
        101,
        logarithmicBucketCapacityFunc
    );
    let uniformSaturations = uniformPopularityBase(
        saturationsPopularityMapObject,
        numColors
    );
    let saturations = averageArrays(
        medianSaturations.average,
        uniformSaturations,
        1.2
    );
    ...
    let vibrantHueFunc = pixel => {
        ...
        const lightnessCeil = Math.min(
            232,
            lightnesses[lightnesses.length - 2]
        );
        if (lightness <= lightnessFloor || lightness >= lightnessCeil) {
            return null;
        }
        //also ignore hue if saturation is too low to distinguish hue
        //TODO: pick between saturation floors - some images look better with one than the other
        const satuarationFloor = Math.max(
            5,
            saturations[Math.floor(saturations.length / 2)]
        );
        if (PixelMath.saturation(pixel) <= satuarationFloor) {
            return null;
        }
    ...
    if (hueMix < 2) {
        let huesUniform = hueUniformPopularity(
            huePopularityMapObject,
            numColors
        );
...
function perceptualMedianCut3(
    ...
    let huePopularityMapObject = createPopularityMap(pixels, 360, hueFunc);
    huePopularityMapObject = filterHues(
        huePopularityMapObject,
        imageHeight * imageWidth
    );
    let huesMedian = calculatedPopularityHues(
        huePopularityMapObject,
        numColors
    );
    let hueMix = colorQuantization.hueMix;
...
function logarithmicEdgeDistribution(numColors, valueMin, valueMax) {
    ...
    for (let exponent = 0, i = halfColors; i < numColors; exponent++, i++) {
        ret[i] = Math.round(
            valueMax - Math.floor(highDiff / Math.pow(highBase, exponent))
        );
    }
    return ret;
}
 ...
 */
function logarithmicEdgeLinearMiddleDistribution(
    numColors,
    valueMin,
    valueMax
) {
    ...
    const multiplier =
        ...
        2 /
        (numColors -
            (bottomQuarterColors + (numColors - topQuarterColors)) +
            1);
    for (
        let index = 1, i = bottomQuarterColors;
        i < topQuarterColors;
        i++, index++
    ) {
    ...
    const highBase =
        Math.log(highDiff) / Math.log(numColors - topQuarterColors);
    for (
        let exponent = 0, i = topQuarterColors;
        i < numColors;
        exponent++, i++
    ) {
        ret[i] = Math.round(
 ...
 */
function logarithmicCenterDistribution(
    numColors,
    valueMin,
    valueMax,
    valueAverage
    ...
    const centerValue = Math.floor(
        ((valueMax - valueMin) / 2 + valueAverage) / 2
    );
    const ret = new Uint8Array(numColors);
    const halfColors = Math.floor(numColors / 2);
    ...
    for (
        let logIndex = Math.ceil(numColors / 2), i = halfColors;
        i < numColors;
        i++, logIndex--
    ) {
...
function perceptualMedianCut4(
    ...
    const hslPopularityMap = createHslPopularityMap(
        pixels,
        colorQuantization.hueClamp
    );
    //lightness
    ...
    const saturations = logarithmicEdgeDistribution(
        numColors,
        saturationStats.min,
        saturationStats.max
    );
...
function perceptualMedianCut5(
    ...
    const lightnesses = logarithmicCenterDistribution(
        numColors,
        lightnessStats.min,
        lightnessStats.max,
        lightnessStats.average
    );

    //saturation
    const saturationStats = hslPopularityMap.saturation;
    const saturationDistributionFunc =
        saturationStats.average > 30
            ? logarithmicEdgeLinearMiddleDistribution
            : logarithmicEdgeDistribution;
    const saturations = saturationDistributionFunc(
        numColors,
        saturationStats.min,
        saturationStats.max
    );
    ...
    huePopularityMapObject = filterHues(
        huePopularityMapObject,
        imageHeight * imageWidth,
        colorQuantization.hueFilterLog
    );
    let huesMedian = calculatedPopularityHues(
        huePopularityMapObject,
        numColors
...
function uniformQuantization(
    ...
    let lightnesses = lightnessUniformPopularity(
        lightnessesPopularityMapObject,
        numColors
    );
    ...
    let saturations = uniformPopularityBase(
        saturationsPopularityMapObject,
        numColors
    );
    ...
    let vibrantHueFunc = pixel => {
        ...
        const lightnessCeil =
            lightnesses[lightnesses.length - lightnessesFraction];
        if (lightness <= lightnessFloor || lightness >= lightnessCeil) {
            return null;
        }
        //also ignore hue if saturation is too low to distinguish hue
 ...
 */
function monochromeQuantization(
    pixels,
    numColors,
    colorQuantization,
    _imageWidth,
    ...
    const lightnesses = lightnessUniformPopularity(
        hslPopularityMap.lightness.mapObject,
        numColors
    );
    ...
    const mostPopularHue = hslPopularityMap.hue.map.reduce(
        (currentMostPopularHueIndex, currentHueCount, currentHueIndex) => {
    ...
    if (hueCount) {
        ...
        const tonesCount = isDynamicTonesCount
            ? Math.ceil(numColors / Math.abs(hueCount))
            : hueCount;
        ...
        for (
            let i = hueStartIndex, toneIndex = 0;
            i < hues.length + hueStartIndex;
            i++, toneIndex++
        ) {
